...
from shapely.geometry import Point, Polygon, MultiPolygon, LineString, MultiLineString, LinearRing, MultiPoint
import shapely
...
class LngLatTransfer:
    ...
    def loc_convert(self, lng, lat, con_type='gc2bd'):
        if con_type == 'gc-bd':
            return self.GCJ02_to_BD09(lng, lat)
        elif con_type == 'gc-84':
            return self.GCJ02_to_WGS84(lng, lat)
            ...
            if isinstance(geo_obj, (LineString, LinearRing)):
                ...
                if len(coords_list) > 1:
                    return Polygon([self.loc_convert(x, y, con_type) for (x, y) in coords_list[0]],
                                   holes=[[self.loc_convert(x, y, con_type) for x, y in ring_coord] for ring_coord in
                                          coords_list[1:]])
                else:
                    return Polygon([self.loc_convert(x, y, con_type) for (x, y) in coords_list[0]])
